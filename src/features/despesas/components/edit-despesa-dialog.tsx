...
export function EditDespesaDialog({ condId, despesaId, open, onOpenChange }: EditDespesaDialogProps) {
  ...
  }, [open, despesaId, condId, form, setInitialAttachments, resetFiles, onOpenChange]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Altere os dados ou adicione novos comprovantes.</DialogDescription>
        </DialogHeader>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormField control={form.control} name="nome" render={({ field }) => (
                <FormItem><FormLabel>Nome da Despesa</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )} />
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField control={form.control} name="tipo" render={({ field }) => (
                  <FormItem><FormLabel>Tipo</FormLabel>
                    ...
                    <SelectContent>{DESPESA_TIPOS.map((t) => (<SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>))}</SelectContent>
                  </Select><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="formaPagamento" render={({ field }) => (
                  <FormItem><FormLabel>Pagamento</FormLabel>
                    <Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger></FormControl>
                    <SelectContent>{FORMA_PAGAMENTO.map((f) => (<SelectItem key={f.value} value={f.value}>{f.label}</SelectItem>))}</SelectContent>
                  </Select><FormMessage /></FormItem>
                )} />
              </div>

              <div className="grid grid-cols-1 gap-4">
                <FormField control={form.control} name="idImovel" render={({ field }) => (
                  <FormItem><FormLabel>Imóvel (Opcional)</FormLabel><FormControl><Input placeholder="Ex: AP 302" {...field} value={field.value || ''} /></FormControl><FormMessage /></FormItem>
                )} />
              </div>
              ...
              <div className="space-y-2">
                ...
                {existingAttachments.length > 0 && (
                  <div className="mt-2 space-y-2">
                    <span className="text-xs text-muted-foreground font-semibold uppercase">Já anexados</span>
                    {existingAttachments.map((file) => (
                      <div key={file.id} className="flex justify-between items-center bg-secondary p-2 rounded text-sm">
                        <div className="flex items-center gap-2 truncate">
                          <FileText className="h-4 w-4 shrink-0 text-muted-foreground" />
                          <span className="truncate">{file.name}</span>
                        </div>
                        <X className="h-4 w-4 cursor-pointer text-destructive shrink-0" onClick={() => removeExistingAttachment(file.id)} />
                      </div>
                    ))}
                  </div>
                )}
                ...
                {files.length > 0 && (
                  <div className="mt-2 space-y-2">
                    <span className="text-xs text-muted-foreground font-semibold uppercase">Novos arquivos</span>
                    {files.map((file: File, index: number) => (
                      <div key={index} className="flex justify-between bg-secondary p-2 rounded text-sm">
                        <span className="truncate">{file.name}</span>
