...
const financeRadarFormatter: React.ComponentProps<
...
const countRadarFormatter: React.ComponentProps<
  typeof ChartTooltipContent
>['formatter'] = (_, __, item) => {
  ...
  return (
    <div className="flex w-full items-center justify-between gap-2">
      ...
      <span className="font-mono">{numberFormatter.format(payload.total)}</span>
    </div>
  );
};
...
function groupByKey<T>(items: T[], key: keyof T): [string, number][] {
...
function toRadarScores(entries: [string, number][]): CountRadarPoint[] {
...
function buildFinanceRadarData(
  cobrancas: CobrancaSummary[],
  totalGastosPagamentos: number,
  totalGastosDespesas: number
): FinanceRadarPoint[] {
  ...
  const metrics = [
    { indicador: 'Receita recebida', valor: sumByStatus('pago') },
    { indicador: 'Receita a receber', valor: sumByStatus('pendente') },
    { indicador: 'Folha salarial', valor: totalGastosPagamentos },
    { indicador: 'Despesas gerais', valor: totalGastosDespesas },
    { indicador: 'Inadimplência', valor: sumByStatus('vencida') },
  ];
...
function getLineStyle(dataKey: string, activeKey: string) {
  const isActive = dataKey === activeKey;
  return {
...
export function DashboardClient({
  ...
  const totalGastosPagamentos = pagamentos.reduce(
    (acc, item) => acc + item.value,
    0
  );
  const totalGastosDespesas = despesas.reduce(
  ...
  const chartData = useMemo<ChartPoint[]>(
    () => buildChartData(pagamentos, despesas, cobrancas),
  ...
  const despesasPorTipo = useMemo(
    () => groupByKey(despesas, 'tipo'),
    [despesas]
  );
  const cobrancasPorStatus = useMemo(
    () => groupByKey(cobrancas, 'status'),
    [cobrancas]
  );
  const funcionariosPorStatus = useMemo(
    () => groupByKey(funcionarios, 'status'),
    [funcionarios]
  );
  const funcionariosPorCargo = useMemo(
    () => groupByKey(funcionarios, 'role'),
    [funcionarios]
  );
  const imoveisPorTipo = useMemo(() => groupByKey(imoveis, 'tipo'), [imoveis]);
  const imoveisPorSituacao = useMemo(
    () => groupByKey(imoveis, 'situacao'),
    [imoveis]
  );
  ...
  const financeRadarData = useMemo<FinanceRadarPoint[]>(
    () =>
      buildFinanceRadarData(
        cobrancas,
        totalGastosPagamentos,
        totalGastosDespesas
      ),
  ...
  const cobrancasStatusRadarData = useMemo<CountRadarPoint[]>(
    () => toRadarScores(cobrancasPorStatus),
    [cobrancasPorStatus]
  );

  const despesasTipoRadarData = useMemo<CountRadarPoint[]>(
    () => toRadarScores(despesasPorTipo),
    [despesasPorTipo]
  );
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
      <div className="border-border/70 bg-card overflow-hidden rounded-xl border">
        <div className="from-primary/70 via-primary/40 to-primary/10 h-1 w-full bg-gradient-to-r" />
        <div className="flex flex-col items-center gap-4 p-6 text-center">
          <div className="flex flex-col items-center gap-4">
            <div className="border-border/70 rounded-xl border bg-white p-2 shadow-sm">
              <Image
                src="/logo-icon.png"
                alt="Moratta"
                width={36}
            ...
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground mx-auto max-w-2xl text-sm md:text-base">
                Bem-vindo ao painel Moratta, a central administrativa do seu
                condomínio. Aqui você acompanha receitas, despesas, equipe,
                imóveis e indicadores estratégicos em um só lugar.
              </p>
            </div>
          </div>
        </div>
      ...
      <RoleGuard roles={['Admin', 'Financeiro', 'RH']}>
        <div className="border-border/70 bg-card rounded-xl border p-5">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <div className="border-border/70 bg-card/80 rounded-lg border p-4">
        ...
        <Card>
          <CardHeader className="flex flex-col items-stretch border-b p-0 sm:flex-row">
            <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-4">
              ...
              <p className="text-muted-foreground text-sm">
                Gastos = pagamentos de funcionários + despesas. Lucro =
                cobranças.
              </p>
            </div>
                ...
                <ChartContainer config={chartConfig} className="h-72 w-full">
                  ...
                  >
                    ...
                    <Line
                      ...
                      {...getLineStyle('lucro', activeFinanceChart)}
                      dot={false}
                    />
                    <Line
                      type="monotone"
                      ...
                      {...getLineStyle('gastos', activeFinanceChart)}
                      dot={false}
                    />
                    <Line
                      type="monotone"
                      ...
                      {...getLineStyle('saldo', activeFinanceChart)}
                      dot={false}
                    />
                  </LineChart>
                </ChartContainer>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                Radar de equilíbrio financeiro
              </CardTitle>
            </CardHeader>
            <CardContent>
              <ChartContainer
                config={financeRadarChartConfig}
              ...
              >
                <RadarChart data={financeRadarData}>
                  ...
                  <ChartTooltip
                    content={
                      <ChartTooltipContent formatter={financeRadarFormatter} />
                    }
                  />
                  <Radar
                    dataKey="score"
          ...
          <Card>
            ...
            <CardContent>
              {cobrancasStatusRadarData.length === 0 ? (
                ...
                <>
                  <ChartContainer
                    config={cobrancaStatusRadarChartConfig}
                    className="h-72 w-full"
                  >
          ...
          <Card>
            ...
            <CardContent>
              {despesasTipoRadarData.length === 0 ? (
                ...
                <>
                  <ChartContainer
                    config={despesaTipoRadarChartConfig}
                    className="h-72 w-full"
                  >
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          ...
          <div className="grid gap-4">
            <BreakdownCard
              ...
              data={funcionariosPorStatus}
            />
            <BreakdownCard
              title="Funcionários por cargo"
              data={funcionariosPorCargo}
            />
          </div>
        </div>
      </RoleGuard>
      ...
      <RoleGuard roles={['Admin']}>
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <BreakdownCard
            title="Imóveis por situação"
            data={imoveisPorSituacao}
          />
        </div>
      </RoleGuard>
    </div>
